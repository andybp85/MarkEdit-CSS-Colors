...
export const BLACK = { a: 1, b: 0, g: 0, r: 0 };
export const WHITE = { a: 1, b: 255, g: 255, r: 255 };
...
export function parseColor(source) {
    ...
    if (form === 'rgb' || form === 'rgba')
        return parseRgbChannels(args.channels, a);
    if (form === 'hsl' || form === 'hsla')
        return parseHslChannels(args.channels, a);
    return undefined;
}
export function luminance({ b, g, r }) {
    return 0.2126 * linearize(r) + 0.7152 * linearize(g) + 0.0722 * linearize(b);
}
/**
 * Black or white, whichever reads on `color` once it is laid over `background`.
 */
export function contrastColor(color, background) {
    return luminance(over(color, background)) > LUMINANCE_THRESHOLD ? '#000000' : '#ffffff';
}
export function isDarkColor(color) {
    return luminance(color) <= LUMINANCE_THRESHOLD;
}
export function toCssColor({ a, b, g, r }) {
    return `rgba(${r}, ${g}, ${b}, ${a})`;
}
/**
 ...
 */
export function findColors(text, options = {}) {
    const found = [];
    ...
    for (let match = CANDIDATE.exec(text); match !== null; match = CANDIDATE.exec(text)) {
        ...
        if (source.startsWith('#') && refusesHexLead(text, from, options))
            continue;
        ...
        if (isWordish(text[to]))
            continue;
        const color = parseColor(source);
        if (color !== undefined)
            found.push({ color, from, to });
    }
    return found;
}
...
function over(color, background) {
    if (color.a >= 1)
        return { a: 1, b: color.b, g: color.g, r: color.r };
    return {
        a: 1,
...
function parseArgs(inner) {
    ...
    if (channels.length === 4 && alpha === undefined)
        alpha = channels.pop();
    if (channels.length !== 3 || channels.some(part => part === '') || alpha === '')
        return undefined;
...
function parseRgbChannels(channels, a) {
    const [r, g, b] = channels.map(rgbChannel);
    if (r === undefined || g === undefined || b === undefined)
        return undefined;
    return { a, b, g, r };
}
function parseHslChannels(channels, a) {
...
// heading.
function refusesHexLead(text, index, options) {
    if (isWordish(text[index - 1]))
        return true;
